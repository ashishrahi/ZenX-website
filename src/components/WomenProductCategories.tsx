...
const WomenProductCategories = ({ title, description }) => {
  ...
  const startAutoplay = () => {
    stopAutoplay();
    if (!WomenCategories.length) return;

    autoplayRef.current = window.setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % WomenCategories.length);
    }, AUTOPLAY_INTERVAL);
  };
  ...
  const goToPrev = () =>
    pauseAndMaybeResume(() =>
      setActiveIndex(
        (prev) => (prev - 1 + WomenCategories.length) % WomenCategories.length
      )
    );

  /** Calculate card positions for carousel 3D effect */
  const calculateCardTransform = (index: number) => {
    const totalItems = WomenCategories.length;
